...
- Chunk cache made incremental: reload amplification (full rebuild + double-buffered old cache on every embedding batch) was the actual cause of a production OOM, not steady-state size. Now patches only the pages that changed via a pending-pages set.
- Chunk cache made lean: stripped chunk text and page url/title from the resident struct (`CachedChunk`: chunk_id, page_id, embedding, pagerank, quality, compilable, domain). `handler/search.go` hydrates text/url/title/license from SQLite for only the final ranked result set (≤ `limit` rows) after DPP, not the whole corpus. Domain (not the full URL) stays resident because DPP's same-source diversity penalty needs it; a domain string is a few bytes and low-cardinality versus a full URL. Measured via controlled A/B at matched corpus state (144,816 cached chunks, binary swapped and cache re-warmed both ways, same DB): 696MB → ~596MB RSS, ~14% reduction. This is the "shrink the constant factor" step, not the "decouple from corpus size" step — see HNSW below.
...
- **Cache (storage) — brute-force scan still scales with corpus size.** Superseded estimate: at ~2,500 pages / 100K chunks the cache was projected to hit memory pressure around 10K pages. Actual: reached 13.5K pages / 145K embedded chunks (441K total chunks, embed backlog ongoing) before real production pressure, mitigated twice — first an incremental-reload fix (fixed a genuine OOM, was reload amplification not steady-state size), then a lean-cache fix (~14% RSS reduction at fixed corpus size, see Done above). Both are constant-factor wins; the scan itself is still O(corpus size) per query and RSS still grows linearly with chunk count forever. codex's assessment (asked directly, 2026-07): quantization alone doesn't fix this (still scans everything); `sqlite-vec`/`vec1` are both pre-v1 with breaking-change warnings, more integration risk than warranted as a first move. The durable fix is an in-process HNSW index for candidate generation — query for top ~200-500 chunk IDs, run the existing exact best-chunk-per-page/PageRank/quality/DPP pipeline on that candidate set instead of the full corpus, keep SQLite as source of truth. Candidates: [fogfish/hnsw](https://github.com/fogfish/hnsw) or [TFMV/hnsw](https://github.com/TFMV/hnsw) (both MIT, pure Go). Parts bin: Cache × embedding_space → HNSW index. **Trigger to revisit**: next OOM, or RSS baseline crosses ~1.5GB on whatever instance size is current, or chunk count roughly doubles from today's 145K embedded. Re-measure with the same controlled-A/B method used for the lean-cache fix (swap binary, re-warm cache, compare RSS at matched corpus state) before deciding scope.
